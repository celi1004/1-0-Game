...
public class GameView extends View {
    ...
    private void init(){
        try{
            ...
            for(int x=3; x<mCellRectangle.length; x++){
                mCellRectangle[x] = getResources().getDrawable(R.drawable.cell_rectangle_4);
            }
    ...
    public void onDraw(Canvas canvas){
        ...
        else if(!(mGameState != Game.State.WON && mGameState != Game.State.LOSE) && mRefreshLastTime){
            invalidate();
    ...
    private void drawTiles(Canvas canvas) {
        ...
        for (int xx = 0; xx < mNumberOfSquaresX; xx++) {
            for (int yy = 0; yy < mNumberOfSquaresY; yy++) {
                ...
                Tile currentTile = mGameGrid.getTileContent(xx, yy);
                if (currentTile != null) {
    ...
    private void createBitmapCells() {
        mPaint.setTextAlign(Paint.Align.CENTER);
        for (int xx = 0; xx < mBitmapCell.length-1; xx++) {
            int value = (int) xx;
            ...
            drawDrawable(canvas, mCellRectangle[xx+1], 0, 0, mCellSize, mCellSize);
    ...
    public void mergedTile(int x, int y) {
